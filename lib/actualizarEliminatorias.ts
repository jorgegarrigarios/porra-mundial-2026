...
export type ActualizarCrucesEliminatoriasOptions = {
  dryRun?: boolean;
  force?: boolean;
};

export type ResultadoActualizarCrucesEliminatorias = {
  ok: boolean;
  error?: string;
  errores?: string[];
  resumen?: {
    ...
    fixturesEliminatoriasDetectados: number;
    fixturesEliminatoriasConEquipos: number;
    horariosReferencia: number;
    gruposCerrados: number;
    gruposCerradosDetalle: Array<{
      grupo: string;
      primero: string;
      segundo: string;
      tercero: string | null;
    ...
    tercerosPendientesDetalle: Array<{
      partidoId: number;
      campo: "local" | "visitante";
      placeholder: string;
    }>;
    crucesFuturosPendientes: number;
    crucesFuturosPendientesDetalle: Array<{
      partidoId: number;
      campo: "local" | "visitante";
      placeholder: string;
    }>;
...
export async function actualizarCrucesEliminatorias(
  options: ActualizarCrucesEliminatoriasOptions = {}
): Promise<ResultadoActualizarCrucesEliminatorias> {
  ...
  if (erroresFixtures || erroresStandings) {
    const error = `Football API ha devuelto errores: ${[
      erroresFixtures,
      erroresStandings,
    ]
      .filter(Boolean)
